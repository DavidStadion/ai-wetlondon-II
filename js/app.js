...
function switchTab(tabName, ev) {
    ...
    // Update tab buttons (support both old/new class names)
    document.querySelectorAll('.modal-tab, .activity-tab').forEach(tab => tab.classList.remove('active'));
    ...
    // Lazy-load Gallery photos when user opens the Gallery tab
    if (tabName === 'gallery') {
        try {
            if (window.currentActivity) populateGalleryFromPlaces(window.currentActivity);
        } catch (err) {
