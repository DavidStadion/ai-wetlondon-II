...
function observeCardsForLazyLoading(container) {
  ...
  const cards = container.querySelectorAll(
    ".activity-card[data-venue-name]:not([data-image-loaded])",
  );
...
async function fetchUnsplashImage(venueName) {
  ...
  const query =
    searchTerms[venueName] ||
    (venueName || "").replace(/, London$/i, "").trim() + " London";
...
function showSearchSuggestions(query) {
  ...
  searchSuggestions.innerHTML = topMatches
    .map((venue) => {
      const nameHighlighted = highlightMatch(venue.name, query);
      const typeLabel =
        venue.type[0].charAt(0).toUpperCase() + venue.type[0].slice(1);
      const locationLabel =
...
function searchVenues(query) {
  ...
  return window.londonVenues
    .filter((venue) => {
      ...
      // Search in types
      if (venue.type.some((type) => type.toLowerCase().includes(lowerQuery)))
        return true;
      ...
      if (
        venue.prerequisites &&
        venue.prerequisites.some((prereq) =>
...
function renderDavidsTopPicks() {
  ...
  const venues = (window.londonVenues || []).filter(
    (v) => v.highlighted === true,
  );
...
function performSearch() {
  ...
  setTimeout(async () => {
    ...
    document.querySelector(".generated-title").textContent =
      `Search: "${query}"(${resultText})`;
...
function createActivityCardHTML(venue, index, options = {}) {
  ...
  const umbrellaEmoji =
    venue.wetness === "slightly" ? "☂️" : venue.wetness === "wet" ? "☔️" : "🌂";
  const tooltipText =
...
function updatePrereqCounter() {
  document.getElementById("prereqCount").textContent =
    `(${selectedPrerequisites.length})`;
}
...
document.querySelectorAll(".location-option").forEach((option) => {
  option.addEventListener("click", function () {
    ...
    if (location === "all") {
      ...
      document
        .querySelector('.location-option[data-location="all"]')
...
document.querySelectorAll(".wetness-option").forEach((option) => {
  option.addEventListener("click", function () {
    ...
    if (wetness === "any") {
      ...
      document
        .querySelector('.wetness-option[data-wetness="any"]')
        .classList.remove("selected");

      // Deselect all other options first (single selection)
      document
        .querySelectorAll('.wetness-option:not([data-wetness="any"])')
        .forEach((opt) => {
          opt.classList.remove("selected");
        });
...
let savedActivities = JSON.parse(
...
function openActivityModal(venue) {
  ...
  if (hero) {
    const heroUrl =
      venue.cardImage ||
      (typeof getCachedImage === "function"
        ? getCachedImage(venue.name)
        : null) ||
      (typeof getPlaceholderImage === "function"
        ? getPlaceholderImage(venue)
        : null);
  ...
  if (venue.affiliateLink) {
    ...
    if (overviewBookBtn) {
      overviewBookBtn.innerHTML =
        '<span class="btn-icon">🎟️</span> Book Tickets';
    ...
    if (overviewBookBtn) {
      overviewBookBtn.innerHTML =
        '<span class="btn-icon">🔍</span> Find Tickets';
      overviewBookBtn.onclick = bookAction;
    }
  }
  ...
  if (modalWetnessLabel)
    modalWetnessLabel.textContent = `${wetnessPercent}% wet`;
  ...
  const hasWheelchair =
    venue.prerequisites &&
    venue.prerequisites.includes("wheelchair accessible");
  const hasStepFree =
    venue.prerequisites && venue.prerequisites.includes("step-free");
  const hasLift =
    venue.prerequisites && venue.prerequisites.includes("lift access");
  let accessText = [];
  ...
  document.getElementById("activityAccessibility").textContent =
    accessText.join(" • ");

  // Set booking info
  const needsBooking =
    venue.prerequisites && venue.prerequisites.includes("booking required");
  const walkInsWelcome =
    venue.prerequisites && venue.prerequisites.includes("walk-ins welcome");
  let bookingText = "";
  ...
  if (venue.openingHours) {
    ...
    document.getElementById("activityOpeningHours").textContent =
      formattedHours + statusText;
  } else {
    document.getElementById("activityOpeningHours").textContent =
  ...
  // Set duration
  const hasFullDay =
    venue.prerequisites && venue.prerequisites.includes("full day");
  const hasHalfDay =
    venue.prerequisites && venue.prerequisites.includes("half day");
  const hasUnder1Hour =
    venue.prerequisites && venue.prerequisites.includes("under 1 hour");
  let durationText = "";
  ...
  if (hasFullDay) {
    ...
    if (venue.type.includes("museums") || venue.type.includes("historic")) {
      ...
      venue.type.includes("gaming") ||
      venue.type.includes("entertainment")
    ) {
  ...
  if (venue.prerequisites && venue.prerequisites.includes("cafe on-site"))
    highlights.push("On-site café");
  if (venue.prerequisites && venue.prerequisites.includes("toilets available"))
    highlights.push("Facilities available");
  if (venue.prerequisites && venue.prerequisites.includes("gift shop"))
    highlights.push("Gift shop");
  if (venue.prerequisites && venue.prerequisites.includes("lockers available"))
  ...
  if (venue.prerequisites && venue.prerequisites.includes("booking required")) {
    ...
    venue.prerequisites &&
    venue.prerequisites.includes("walk-ins welcome")
  ) {
  ...
  if (
    venue.prerequisites &&
    venue.prerequisites.includes("photography allowed")
  ) {
...
function switchTab(tabName) {
  ...
  const activeIndicator = document.querySelector(
    `.tab-indicator[data-tab="${tabName}"]`,
  );
...
async function bookmarkActivity() {
  ...
  const index = savedActivities.findIndex(
    (v) => v.name === currentActivity.name,
  );
...
function showBookmarkToast(wasAdded) {
  ...
  toastMessage.textContent = wasAdded
...
function shareActivity() {
  ...
  document.getElementById("shareActivityName").textContent =
    currentActivity.name;
  document.getElementById("shareActivityDescription").textContent =
...
function openBookingOptions(venueName, viatorUrl, gyguUrl) {
  ...
  if (bookingModal) {
    ...
    document.getElementById("googleSearchLink").href =
      `https://www.google.com/search?q=${encodeURIComponent(venueName + " London tickets book")}`;
    ...
    btn.onclick = function (e) {
      ...
      if (!venue) {
        const description =
          ...
          "A wonderful London activity perfect for any weather.";
        const priceText =
          card.querySelector(".price")?.textContent ||
          "Check venue for pricing";
        const locationText =
          card.querySelector(".tag:nth-child(2)")?.textContent ||
          "Central London";
        const typeText =
          card.querySelector(".tag:nth-child(1)")?.textContent ||
          "entertainment";
        const wetnessText =
          card.querySelector(".tag:nth-child(3)")?.textContent || "dry";
    ...
    card.onclick = function (e) {
      ...
      if (!venue) {
        ...
        const priceText =
          ...
          "Check venue for pricing";
        const locationText =
          this.querySelector(".tag:nth-child(2)")?.textContent ||
          "Central London";
        const typeText =
          this.querySelector(".tag:nth-child(1)")?.textContent ||
          "entertainment";
        const wetnessText =
          this.querySelector(".tag:nth-child(3)")?.textContent || "dry";
...
async function toggleBookmarkFromCard(event, venueName) {
  ...
  const bookmarkIcon =
    ...
    event.currentTarget.classList.contains("bookmark-icon")
      ? event.currentTarget
      : event.target && event.target.closest
        ? event.target.closest(".bookmark-icon")
        : null;
  ...
  if (!venue) {
    ...
    if (card) {
      const description =
        ...
        "A wonderful London activity perfect for any weather.";
      const priceText =
        card.querySelector(".price")?.textContent || "Check venue for pricing";
      const locationText =
        card.querySelector(".tag:nth-child(2)")?.textContent ||
        "Central London";
      const typeText =
        card.querySelector(".tag:nth-child(1)")?.textContent || "entertainment";
      const wetnessText =
        card.querySelector(".tag:nth-child(3)")?.textContent || "dry";
...
async function showBookmarks() {
  ...
  if (savedActivities.length === 0) {
    ...
    const imagePromises = savedActivities.map((venue) =>
      fetchVenueImage(venue.name),
    );
    const images = await Promise.all(imagePromises);

    grid.innerHTML = savedActivities
      .map((venue, index) => {
...
async function showRecentlyViewed() {
  ...
  setTimeout(async () => {
    const imagePromises = recentlyViewed.map((venue) =>
      fetchVenueImage(venue.name),
    );
    const images = await Promise.all(imagePromises);
...
function initializeCategoryChips() {
  ...
  // Open now chip count
  const openCount = (window.londonVenues || []).reduce(
    (acc, v) => acc + (isVenueOpenNow(v) === true ? 1 : 0),
    0,
  );
  ...
  const openChipEl = document.querySelector(
    '.category-chip[data-category="__openNow"]',
  );
...
function toggleCategoryFilter(category) {
  ...
  if (category === "__openNow") {
    ...
    if (clearBtn)
      clearBtn.style.display =
        activeCategoryFilters.length > 0 || filters.openNow ? "block" : "none";
  ...
  const chip = document.querySelector(
    `.category-chip[data-category="${category}"]`,
  );
  const clearBtn = document.getElementById("clearCategoryFilters");
...
async function applyAllActivitiesFilters() {
  ...
  if (activeCategoryFilters.length === 0) {
    ...
    allActivitiesFiltered = window.londonVenues.filter((venue) => {
      const matchesFilter = venue.type.some((type) =>
        activeCategoryFilters.includes(type),
      );
      if (matchesFilter) {
    ...
    console.log(
      "Filtered venue names:",
      allActivitiesFiltered.map((v) => v.name),
    );
  }
  // Apply Open now chip filter
  if (filters.openNow) {
    allActivitiesFiltered = allActivitiesFiltered.filter(
      (v) => isVenueOpenNow(v) === true,
    );
    console.log(
      "Applied open now filter, count:",
      allActivitiesFiltered.length,
    );
  }
...
async function renderAllActivities(append = false) {
  ...
  const imagePromises = venuesToShow.map((venue) =>
    fetchVenueImage(venue.name),
  );
  const images = await Promise.all(imagePromises);
...
async function sortAllActivities() {
  ...
  switch (sortValue) {
    ...
    case "price-desc":
      ...
      break;
    case "location":
      allActivitiesFiltered.sort((a, b) =>
        a.location.localeCompare(b.location),
      );
      break;
    case "wetness-asc":
      allActivitiesFiltered.sort(
        (a, b) => (a.wetnessScore || 0) - (b.wetnessScore || 0),
      );
      break;
    case "wetness-desc":
      allActivitiesFiltered.sort(
        (a, b) => (b.wetnessScore || 0) - (a.wetnessScore || 0),
      );
      break;
  }
...
function updateActivityStats() {
  // Total count
  document.getElementById("totalActivitiesCount").textContent =
    allActivitiesFiltered.length;

  // Count open now
  const openNow = allActivitiesFiltered.filter(
    (v) => isVenueOpenNow(v) === true,
  ).length;
...
function updateOpenNowBadges() {
  ...
  const featuredSection = document.querySelector(
    ".activities-section .activity-grid",
  );
...
function isVenueOpenNow(venue) {
  ...
  return (
    currentTimeInMins >= openTimeInMins && currentTimeInMins < closeTimeInMins
  );
}
...
function isClosingSoon(venue) {
  ...
  return (
    closeTimeInMins - currentTimeInMins <= 60 &&
    closeTimeInMins - currentTimeInMins > 0
  );
}
...
async function fetchWeather() {
  ...
  try {
    ...
    const response = await fetch(
      "https://api.open-meteo.com/v1/forecast?latitude=51.5074&longitude=-0.1278&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,precipitation",
    );
...
function displayWeather(weather) {
  ...
  messageEl.className = weather.isRaining
    ? "weather-message"
...
async function updateWeatherRecommendations(weather) {
  ...
  if (weather.isRaining) {
    ...
    recommendedVenues = window.londonVenues
      .filter(
        (v) =>
          v.wetness === "dry" &&
          (v.type.includes("dining") ||
            v.type.includes("cinema") ||
            v.type.includes("wellness")),
      )
      .sort((a, b) => b.rating - a.rating)
      .slice(0, 6);
    ...
    recommendedVenues = window.londonVenues
      .filter(
        (v) =>
          ...
          (v.type.includes("galleries") ||
            v.type.includes("shopping") ||
            v.type.includes("exhibitions")),
      )
      .sort((a, b) => b.rating - a.rating)
      .slice(0, 6);
  ...
  if (recommendedVenues.length > 0) {
    ...
    // Build cards HTML
    const cardsHTML = recommendedVenues
      .map((venue, index) => {
        return createActivityCardHTML(venue, index, {
          showId: false,
    ...
    setTimeout(async () => {
      try {
        const imagePromises = recommendedVenues.map((venue) =>
          fetchVenueImage(venue.name),
        );
        const images = await Promise.all(imagePromises);
        ...
        images.forEach((imageUrl, index) => {
          if (imageUrl) {
            const card = grid.querySelector(
              `[data-venue-index="${index}"] .activity-image`,
            );
            if (card) {
        ...
        console.warn(
          "Could not fetch images for weather recommendations:",
          error,
        );
      }
    }, 500);
...
async function loadFeaturedActivityImages() {
  ...
  const cards = featuredSection.querySelectorAll(
...
function toggleQuickFilter(filterType) {
  const chip = document.querySelector(
    `.quick-filter-chip[data-filter="${filterType}"]`,
  );
  if (!chip) return;
  ...
  setTimeout(() => {
    ...
    if (typeof showToast === "function") {
      if (filtered.length > 0) {
        showToast(
          "✨",
          `Found ${filtered.length} ${filtered.length === 1 ? "activity" : "activities"}`,
        );
      } else {
...
async function loadReviews(venueName) {
  ...
  try {
    ...
    const resp = await fetch(
      `/api/place-details?q=${encodeURIComponent(venueName + " London")}`,
    );
    const data = await resp.json();
    ...
    // Update summary
    if (data.rating) {
      document.getElementById("reviewsAvgRating").textContent =
        data.rating.toFixed(1);
      document.getElementById("reviewsStarsLarge").textContent = getStarDisplay(
        data.rating,
      );
    ...
    if (data.reviews && data.reviews.length > 0) {
      googleContainer.innerHTML = data.reviews
        .map(
          (review) => `
      ...
      googleContainer.innerHTML =
        '<p style="color: #999; text-align: center; padding: 1rem;">No Google reviews available yet.</p>';
    }
    ...
    if (googleContainer) {
      googleContainer.innerHTML =
        '<p style="color: #999; text-align: center; padding: 1rem;">Unable to load Google reviews.</p>';
    }
  }
}
...
async function loadUserReviews(venueName) {
  ...
  try {
    const resp = await fetch(
      `/api/reviews?venue=${encodeURIComponent(venueName)}`,
    );
    const data = await resp.json();
    ...
    if (data.reviews && data.reviews.length > 0) {
      ...
      container.innerHTML =
        '<p style="color: #999; text-align: center; padding: 1rem;">No community reviews yet. Be the first!</p>';
    }
  } catch (error) {
...
async function submitReview(event) {
  ...
  if (!name || !text || text.length < 10) {
    errorMsg.textContent =
      "Please fill in all fields. Review must be at least 10 characters.";
...
function createPartnerCardHTML(partner, index) {
  ...
  const locationLabel = partner.location
    ? partner.location.charAt(0).toUpperCase() +
      partner.location.slice(1) +
      " London"
    : "London";
...
async function renderSmallMightyPartners() {
  ...
  try {
    ...
    grid.innerHTML = partners
      .map((partner, index) => createPartnerCardHTML(partner, index))
      .join("");
